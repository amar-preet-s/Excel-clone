...
function addEventsToCells() {
    ...
    $(".input-cell").click(function(e){
        let [rowId,colId] = findRowCol(this);
        let [topCell,bottomCell,leftCell,rightCell] = TopBottomLeftRightCell(rowId,colId);
...
function TopBottomLeftRightCell(rowId,colId){
...
function SelectAllBetweenTheRange(start,end){
    for(let i=(start.rowId<end.rowId?start.rowId:end.rowId); i<=(start.rowId>end.rowId?start.rowId:end.rowId); i++){
        for(let j=(start.colId<end.colId?start.colId:end.colId); j<=(start.colId>end.colId?start.colId:end.colId); j++){
            let [topCell,bottomCell,leftCell,rightCell] = TopBottomLeftRightCell(i,j);  
...
$(".color-picker").colorPick({
    ...
    'onColorSelected': function() {
      if(this.color!='#TYPECOLOR'){
        let ele_border = $(this.element[0]).attr("id");
        let key = (ele_border == "fill-color"?"bgcolor":"color");
        let property = (ele_border == "fill-color"?"background-color":"color");
        colorcell(ele_border,key,property,this.color);
        }
    }
  });

  function colorcell(ele_border,key,property,value){
    $(`#${ele_border}-icon`).css("border-bottom",`4px solid ${value}`);
    $(".input-cell.selected").css(property,value);
 ...
 $("#text-color-icon,#fill-color-icon").click(function(e){
    setTimeout(() => {
        $(this).parent().click();
    }, 10); 
 })
...
$(".add-sheet").click(function(e){
    ...
    $(".sheet-tab.selected").removeClass("selected");
    $(".sheet-tab-container").append(`<div class="sheet-tab selected">${selectedSheet}</div>`);
    addSheetTabEventListeners();
    $("#row-1-col-1").click();
    saved = false;
});
...
function addSheetTabEventListeners(){
     // $(".sheet-tab").off("bind","click"); no need to off previous event listener if we apply new even listener only on new tab
...
$(".left-scroller").click(function(){
    let keysArray = Object.keys(cellData);
    selectedSheetIndex = keysArray.indexOf(selectedSheet);
    if(selectedSheetIndex!=0){
